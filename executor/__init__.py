...
class Executor(object):
    ...
    def createTrade(self, ep, trade_id, order_event):
        ...
        self.tran.addTrade(oe.id, event)
    ...
    def _checkOrders(self, event):
        ...
        for _ in range(quelen):
            _id, order_event = self.tran.popOrder()
            self.checkOrder(ep, order_event, bid, ask)
            if order_event.status >= env.ESTATUS_ORDER_CLOSED:
                self.tran.addOrderHistory(_id, order_event)
            else:
                self.tran.addOrder(_id, order_event)
    ...
    def _checkTrades(self, event):
        ...
        for _ in range(quelen):
            ...
            if trade_event.status >= env.ESTATUS_TRADE_CLOSED:
                lib.printInfo(trade_event.endep, "tradeC id=%d %.3f profit=%.3f" % \
                ...
                self.tran.addTradeHistory(_id, trade_event)
            else:
                self.tran.addTrade(_id, trade_event)
